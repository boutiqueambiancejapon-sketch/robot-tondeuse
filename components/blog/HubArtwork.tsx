/**
 * HubArtwork — illustration thématique par hub/article (Atelier Vert).
 *
 * Stratégie : photo réelle si disponible dans SLUG_PHOTOS, sinon SVG génératif.
 * Server Component — SVG inline, zéro JS.
 */

import Image from 'next/image'

/** Photos jardins disponibles — slug → chemin public */
...
export function HubArtwork({ slug, variant = 'card', className }: HubArtworkProps) {
  // Photo réelle disponible → on l'utilise directement
  const photoSrc = SLUG_PHOTOS[slug]
  if (photoSrc) {
    return (
      <Image
        src={photoSrc}
  ...
  // Fallback : illustration SVG générative
...
function TechComposition({ w, h, accent, variant }: CompProps) {
  ...
  return (
    <g>
      {[0, 1, 2, 3, 4].map((i) => (
        <circle key={i} cx={cx} cy={cy} r={50 + i * 35} fill="none" stroke={accent} strokeWidth="1.2" strokeDasharray="4 6" opacity={0.35 - i * 0.05} />
      ))}
      {[0, 120, 240].map((deg, i) => {
        const r = 130
    ...
    <g>
      ...
      <g transform={`translate(${cx + 140}, ${cy - 80})`}>
        {Array.from({ length: 8 }).map((_, i) => (
          <rect key={i} x="-3" y="-30" width="6" height="10" fill={accent} transform={`rotate(${i * 45})`} />
        ))}
      ...
      <g transform={`translate(${cx - 100}, ${cy})`}>
        ...
        {Array.from({ length: 12 }).map((_, i) => {
          ...
          return (
            <line
              key={i}
              x1={Math.cos(a - Math.PI / 2) * 50}
              y1={Math.sin(a - Math.PI / 2) * 50}
              x2={Math.cos(a - Math.PI / 2) * 56}
              y2={Math.sin(a - Math.PI / 2) * 56}
              stroke={accent}
              strokeWidth="2"
            />
          )
